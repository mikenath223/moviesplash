import MOVIES_DB from 'moviesplash/config.env';

export default {
  SplashScreenDelayMs: 1 * 700,
};

const { API_KEY } = MOVIES_DB;
...
export const newPageUrl = (url, pageNumber) => {
  const regexDecre = new RegExp(`&page=${pageNumber - 1}`);
  const regexIncre = new RegExp(`&page=${pageNumber + 1}`);
  let newUrl;
